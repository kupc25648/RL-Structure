'''
=====================================================================
Master file for DDPG for training frame structure
This is the master file for using DDPG alogorithm for sturcutral optimization problems.
Adjustable parameter are under '研究室'

フレーム構造をトレーニングするためのDDPGのマスターファイル
これは、構造最適化問題にDDPGアルゴリズムを使用するためのマスターファイルです。
...
game_num_agents= 1 # Number of agent(always 1 for DDPG) / エージェントの数（DDPGの場合は常に1）
...
objective = 'total-suface' # objective of this game / ＃このゲームの目的
...
def run(game,objective,train_period=1):
    ...
    while env1_test.over != 1:
        ...
        if intcounter == 0:
            ...
            save_path = 'DDPG_Model_data_txt_Game{}/'.format(counter+1)
            if not os.path.exists('DDPG_Model_data_txt_Game{}/'.format(counter+1)):
                os.makedirs('DDPG_Model_data_txt_Game{}/'.format(counter+1))
            name = os.path.join(save_path, name_of_file)
    ...
    # 出力モデルファイルの書き込みと保存
    # ------------------------------
    gamename_out1 = 'DDPG_Model_data_txt_Game{}/out{}.txt'.format(counter+1,game)
    gamename_out2 = 'DDPG_Model_data_txt_Game{}/outz{}.txt'.format(counter+1,game)
...
with tf.compat.v1.Session() as sess: # Using tensorflow's calculation / ＃tensorflowの計算を使用する
    reinforcement_learning = DDPG_Actor_Critic(lr,ep,epd,gamma,a_nn,c_nn,max_mem,num_ob,num_action,sess,mu,theta,sigma) # Make an agent / エージェントを作る
    ...
    while counter < num_episodes:
        ...
        game = Game3(game_end_step,game_alpha,game_max_y_val,model,game_num_agents,game_render,game_tell_action) # choose game /ゲームを選択 (研究室)
