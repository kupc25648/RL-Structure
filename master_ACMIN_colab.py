'''
=====================================================================
Master file for Actor-Critic for training frame structure
This is the master file for using Actor-Critic alogorithm for sturcutral optimization problems.
Adjustable parameter are under '研究室'

フレーム構造をトレーニングするためのActor-Criticのマスターファイル
これは、構造最適化問題にActor-Criticアルゴリズムを使用するためのマスターファイルです。
...
# frame_ENV パラメータ
#----------------------------------
game_end_step = 500 # Game end step / ゲーム終了ステップ
...
game_num_agents= 1 # Number of agent(always 1 for Actor-Critic) / エージェントの数（Actor-Criticの場合は常に1）
...
objective = 'total-surface' # objective of this game / ＃このゲームの目的
game_type = 4  #game_type difines number of input of neural network agent [game_type:num_ob , 1:7 , 2:7 , 3:7 , 4:7] / game_typeはニューラルネットワークエージェントの入力数を定義します [game_type:num_ob , 1:7 , 2:7 , 3:7 , 4:7]
#----------------------------------
...
forgame = 0000 # forgame defines what kind of game/ forgameは、どのようなゲームを定義します
#----------------------------------
# frame_RL parameters (Actor-Critic) 研究室
# frame_RL パラメータ (Actor-Critic)
#----------------------------------
...
a_nn = [256,256,256] # Actor ニューラルネットワーク
c_nn = [256,256,256] # Critic ニューラルネットワーク
max_mem = 1000000 # maximum length of replay buffer / 再生バッファの最大長
num_ob = 7 #[num_ob  7] nueral network input(state) (related to game_type) / ニューラルネットワーク入力(状態)（game_typeに関連）
...
def run(game,objective,train_period=1):
    ...
    while env1_test.over != 1:
        ...
        if intcounter == 0:
            ...
            save_path = 'AC_Model_data_txt_Game{}/'.format(counter+1)
            if not os.path.exists('AC_Model_data_txt_Game{}/'.format(counter+1)):
                os.makedirs('AC_Model_data_txt_Game{}/'.format(counter+1))
            name = os.path.join(save_path, name_of_file)
    ...
    # 出力モデルファイルの書き込みと保存
    # ------------------------------
    gamename_out1 = 'AC_Model_data_txt_Game{}/out{}.txt'.format(counter+1,game)
    gamename_out2 = 'AC_Model_data_txt_Game{}/outz{}.txt'.format(counter+1,game)
...
with tf.compat.v1.Session() as sess: # Using tensorflow's calculation / ＃tensorflowの計算を使用する
    reinforcement_learning = Actor_Critic(lr,ep,epd,gamma,a_nn,c_nn,max_mem,num_ob,num_action,sess) # Make an agent / エージェントを作る
    ...
    while counter < num_episodes:
        ...
        game = Game1(game_end_step,game_alpha,game_max_y_val,model,game_num_agents,game_render,game_tell_action) # choose game /ゲームを選択 (研究室)
